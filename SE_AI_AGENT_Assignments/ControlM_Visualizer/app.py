...
if uploaded_file:
    ...
    if not df_jobs.empty:
        ...
        with col3: reg_val = st.text_input("Regex Filter", placeholder="e.g. JOB_A.*, JOB_B.*")
        with col4: 
        ...
        # --- MULTI-PATTERN REGEX LOGIC ---
        if reg_val:
            patterns = [p.strip() for p in reg_val.split(',') if p.strip()]
            for pattern in patterns:
                try:
                    matches = [j for j in all_jobs if re.search(pattern, j, re.IGNORECASE)]
                    # Use set to avoid duplicates immediately, then convert back to list later
                    search_list.extend(matches)
                except: pass
