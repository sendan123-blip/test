...
THEMES = {
    "Dark Mode": {
        ...
        "placeholder": "#bbbbbb",
        "graph_bg": "#121212",
        "nav_btn_filter": "invert(23%) sepia(98%) saturate(7472%) hue-rotate(358deg) brightness(96%) contrast(115%)", # Red Icons
...
# --- 3. RENDERING ENGINE (Injects CSS directly into HTML) ---
def render_pyvis_html(G, search_list, theme_cfg):
    ...
    for n in G.nodes():
        ...
        net.add_node(
            ...
            shape='box', widthConstraint=200, borderWidth=2, margin=12
        )
    ...
    options_dict = {
        "layout": {
            "hierarchical": { "enabled": True, "direction": "LR", "sortMethod": "directed", "nodeSpacing": 200, "levelSeparation": 350 }
        },
        "edges": { "smooth": { "type": "cubicBezier", "forceDirection": "horizontal", "roundness": 0.5 } },
        "physics": { "hierarchicalRepulsion": {"nodeDistance": 220}, "stabilization": {"enabled": True, "iterations": 100} },
        "interaction": { "hover": True, "navigationButtons": True, "keyboard": True, "multiselect": True, "zoomView": True, "selectConnectedEdges": True }
    }
    ...
    # INJECT CUSTOM CSS INTO THE HTML HEAD
    # This solves the "Black Box" issue by styling the container internally
    custom_css = f"""
    <style>
        body {{
            ...
            margin: 0;
            padding: 0;
            /* Create the 'Card' look internally */
            border: 1px solid {theme_cfg['input_border']};
            border-radius: 8px;
            box-sizing: border-box;
        }}
        /* Force Navigation Buttons to be RED/VISIBLE */
        .vis-network .vis-navigation .vis-button {{
            filter: {theme_cfg['nav_btn_filter']} !important;
        }}
        #mynetwork {{
            width: 100%;
            height: 100vh;
...
if uploaded_file:
    ...
    if not df_jobs.empty:
        ...
        with col1: st.markdown("### 🕸️ Lineage")
        ...
        if len(G_disp) > 0:
            ...
            # Generate the HTML string with injected CSS
            final_html = render_pyvis_html(G_disp, search_list, theme)
            
            # Render Single Component - NO WRAPPERS
